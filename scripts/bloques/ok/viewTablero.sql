...
declare
	...
	partida partidas.id_partida%type; -- numero de partida
	jugador1 varchar2(47); -- nombre jugador 1
	jugador2 varchar2(47); -- nombre jugador 2
	nMovimientos number;
	mueven varchar2(7);
	msg varchar2(70) := 'No existe vista de tablero ya que aun no se han cargado partidas...';
begin
	...
	if partida < 0 then
		...
		begin
			...
			dbms_output.put_line(chr(13));
			dbms_output.put_line('.     Partida Numero: '||partida);
			dbms_output.put_line(rpad('.     Blancas: '||jugador1, 42, ' ')||lpad('Negras: '||jugador2, 43, ' '));
			dbms_output.put_line(rpad('.     Movimientos realizados: '||nMovimientos, 42, ' ')||lpad('Mueven: '||mueven, 43, ' '));
			dbms_output.put_line(chr(13));
...
exception
	when others then
		dbms_output.put_line(chr(13));
		dbms_output.put_line('Error Oracle...'||SQLCODE||' detalle: '||SQLERRM);
end;
/